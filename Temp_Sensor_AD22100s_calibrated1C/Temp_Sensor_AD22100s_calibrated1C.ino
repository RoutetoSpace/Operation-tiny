...
unsigned int ADCCalib; //ADC callibrated to approximate temperature
...
void loop()
{
  ...
  ADCCalib = ADCValue - 29;
  Valout = map(ADCValue, 0, 1024, -50, 150); //maps the analogue read output range to the temperature range
  Valout2 = map(ADCCalib, 0, 1024, -50, 150); //maps the corrected analogue read output to the temp range
