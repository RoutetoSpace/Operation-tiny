          ...
          if (newGPSData)
                  {
                           ...
                           sprintf(datastring, "$$RTSHAB-TEST,%i,%s,%s,%s,%s,%i,%i,%i,%i,%d,",msgcount,timechara,latstr,lonstr,ExtTempC,alt,sats,accu,IntTempC,Vcc); // Puts the required data in the datastring  -  removed 3 unnecessary %i's --1/4/13 - added an extra , to string becuase of error with checksum
          ...
          if(newGPSData == false)
                  {
                           ...
                           sprintf(datastring,"$$RTSHAB-TEST,%i,NOGPS,%s,%i,%i,%d,",msgcount,ExtTempC,sats,IntTempC,Vcc);
                           char checksum_str[10];
